...
To do this, we'll load in HGNC's spreadsheet of protein-coding genes, downloaded from [their website](https://www.genenames.org/cgi-bin/statistics), updating gene symbols using `checkGeneSymbols`, and taking only the columns we want. 

```{r universe_init, results='hide', message=FALSE, warning=FALSE}
#we can load directly from website:
#hgnc<- fread("ftp://ftp.ebi.ac.uk/pub/databases/genenames/new/tsv/locus_types/gene_with_protein_product.txt")
# or used version I downloaded (quicker)
...
```{r omim_df_filter2, results='hide', message=FALSE, warning=FALSE}
#updating gene names
...
```{r omim_df_inheritance, results='hide', message=FALSE, warning=FALSE}
...
```{r omim_save, results='hide', message=FALSE, warning=FALSE}
...
```{r exac1}
#using ftp
#exac<- fread("ftp://ftp.broadinstitute.org/pub/ExAC_release/release0.3.1/functional_gene_constraint/fordist_cleaned_exac_r03_march16_z_pli_rec_null_data.txt")
#using downloaded file (quicker)
...
```{r exac2, results='hide', message=FALSE, warning=FALSE }
...
```{r samocha, results='hide', message=FALSE, warning=FALSE }
...
```{r nmd, results='hide', message=FALSE, warning=FALSE}
...
```{r impc, results='hide', message=FALSE, warning=FALSE}
...
```{r cell_ko, results='hide', message=FALSE, warning=FALSE}
...
* ```r length(which(universe_df$omim=="Y"))``` OMIM genes linked to clinically relevant phenotypes show great diversity in scores of genetic constraint.


```{r fig_one_a, fig.align='center',fig.height = 4, fig.width = 8}
#1A: scatter plots
...
* OMIM genes as a group are ```r round(omim_constraint_OR$estimate[[1]],2)``` times more likely to show missense constraint 


```{r fig_one_bi, fig.align='center',fig.height = 4, fig.width = 8}
...
* A slightly larger proportion of OMIM genes have missense constraint, compared to non-OMIM genes (OMIM = ```r round(omim_mis[1,2]/sum(omim_mis[2])*100,1)```% versus non-OMIM = ```r round(nonomim_mis[1,2]/sum(nonomim_mis[2])*100,1)```%)
...
* OMIM genes as a group are ```r round(omim_constraint_OR$estimate[[1]],2)``` times more likely to show LoF constraint 

```{r fig_one_bii, fig.align='center',fig.height = 4, fig.width = 8}
...
* A slightly larger proportion of OMIM genes have LoF constraint, compared to non-OMIM genes (OMIM = ```r round(omim_lof[1,2]/sum(omim_lof[2])*100,1)```% versus non-OMIM = ```r round(nonomim_lof[1,2]/sum(nonomim_lof[2])*100,1)```%)

* The majority of OMIM genes do not exhibit whole-gene genetic constraint (n=```r length(which(universe_df$omim=="Y"&universe_df$constrained=="N"))```/```r length(which(universe_df$omim=="Y"&!is.na(universe_df$constrained)))```, ```r round(length(which(universe_df$omim=="Y"&universe_df$constrained=="N"))/length(which(universe_df$omim=="Y"&!is.na(universe_df$constrained)))*100,digits=1)```%).


#### Levels of genetic constraint correlate with inheritance pattern

* Patterns of missense or LoF constraint correlate broadly with whether the inheritance pattern of the associated phenotype is caused by homozygous, heterozygous or hemizygous mutation 

```{r fig_one_c, fig.align='center',fig.height = 4, fig.width = 8}
...
length(which(universe_df$Inheritance_pattern=="AR,AD"&!is.na(universe_df$exac)))
...
* ```r length(which(universe_df$Inheritance_pattern=="AR"&!is.na(universe_df$exac)))``` OMIM genes cause autosomal recessive (AR) inheritance
    * ```r length(which(universe_df$Inheritance_pattern=="AR"&universe_df$mis_z>=3.09))```/```rlength(which(universe_df$Inheritance_pattern=="AR"&!is.na(universe_df$exac)))``` (```r round(length(which(universe_df$Inheritance_pattern=="AR"&universe_df$mis_z>=3.09))/length(which(universe_df$Inheritance_pattern=="AR"&!is.na(universe_df$exac)))*100,1)```) show missense constraint
    * with 122/1451 (8.4%) of AR genes intolerant to LoF variation.  
* In contrast, of 831/3185 OMIM genes associated with autosomal dominant (AD) disorders; 242/831 (29%) show missense constraint and 384/831 (46%) are intolerant to LoF variation.
* . X-linked (XL) genes (163/3185 OMIM genes) show intermediate levels of missense constraint (20.1% XL compared to 4.3% AR and 29% AD); though are the most intolerant of LoF variation 109/163 (66.9%), consistent with hemizygous expression of X-linked genes in males.  
* Among 36/3185 OMIM nuclear-encoded genes annotated as mitochondrial inheritance (MI), none show missense constraint

```{r fig_one_d, fig.align='center'}
...
* A large proportion of disease genes show neither missense nor LoF constraint (89.8% AR genes, 46.6% of AD genes and 30.7% of XL genes). An unexpected and important finding.

### Figure Two


```{r fig_two_a, fig.align='center'}
