...
a<- a+ggtitle(paste("OMIM genes \n n=",length(which(universe_df$omim=="Y"&!is.na(universe_df$exac)))))+theme(plot.title = element_text(size = 20, face = "bold"),legend.position="none")
...
b<- b+ggtitle(paste("Non-OMIM genes \n n=",length(which(is.na(universe_df$omim)&!is.na(universe_df$exac)))))+theme(plot.title = element_text(size = 20, face = "bold"),legend.position="none")
...
d<- d+ggtitle(paste("Non-OMIM genes \n n=",length(which(is.na(universe_df$omim)&!is.na(universe_df$exac)))))+theme(plot.title = element_text(size = 20, face = "bold"),legend.position="none")


png("Sandra_Figures/Figs/fig1a.png",width=1000,height=1000,type="quartz",res=150,bg = "transparent")
ggarrange(
...
nonomim <- ggplot(universe_df[which(is.na(universe_df$omim)&!is.na(universe_df$exac)),],aes(x=mis_z,y=pLI))+
      ...
      labs(x="Missense Z Score",y="pLI score")+ggtitle("non-OMIM genes\n")+
...
omim <- ggplot(universe_df[which(universe_df$omim=="Y"&!is.na(universe_df$exac)),],aes(x=mis_z,y=pLI))+
  ...
  geom_vline(xintercept=3.09,linetype="dashed",color="lightsalmon2",size=1.5)+
  geom_hline(yintercept=0.9,linetype="dashed",color="indianred3",size=1.5)+
  labs(x="Missense Z Score",y="pLI score")+ggtitle("OMIM genes\n")+
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.01)) +
  scale_x_continuous(expand = c(0, 0), limits = c(-9, 14)) 
